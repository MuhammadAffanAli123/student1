...
def pdb_to_phylip(input="./files/input/example.pdb-seqres", output="./files/output/pdb-phylip.phylip"):
    tmp = "./files/tmp.genbank"
    pdb_to_genbank(input, tmp)
    lines = genbank_to_phylip(tmp, output)
    os.remove(tmp)
    return lines
    #return SeqIO.convert(input, "pdb-seqres", output, "phylip")
...
def main():
        if len(sys.argv) != 4:
                ...
                elif method == "pdb_to_phylip":
                        pdb_to_phylip(input, output)
